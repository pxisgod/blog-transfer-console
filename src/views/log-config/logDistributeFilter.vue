<template>
  <div class="app-container" style="margin:24px 65px;">
    <page-head @back="goBack" content="分发过滤配置"></page-head>
        ...
        <template slot-scope="scope">
          <template v-if="scope.row.edit">
            <el-select v-model="scope.row.paramName" class="filter-item" placeholder="Please select" @change="((val)=>{changeStatus(val, scope.row)})">
              <el-option v-for="item in mapperParamList" :key="item.destColumn" :label="item.destColumn" :value="item.destColumn" />
            </el-select>
          </template>
...
const filterType = "DISTIBUTE_FILTER_TYPE";
const stateType = [
  {
    key: "E",
    display_name: "是"
...
export default {
  ...
  methods: {
    ...
    getList() {
      var loadCount = 2;
      getDistributeMapper(this.$route.query).then(response => {
        if (response.data) {
          this.mapperParamList = response.data.mapperParamList;
        }
        loadCount--;
        if (loadCount == 0) {
      ...
      getDistributeFilter(this.baseInfo).then(response => {
        if (response.data) {
    ...
    handleSave() {
      ...
      saveDistributeFilter(this.baseInfo).then(() => {
        this.goBack();
      });
    },
    changeStatus(val, row) {
      for (var index in this.mapperParamList) {
        if (this.mapperParamList[index].destColumn == val) {
          row.paramType = this.mapperParamList[index].destColumnType;
          break;
        }
      }
    },
