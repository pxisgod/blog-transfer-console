<template>
  <div class="app-container">
    ...
    <el-table :key="tableKey" v-loading="listLoading" :data="list" border fit highlight-current-row style="width: 100%;">
      ...
      <el-table-column label="处理失败量">
        <template slot-scope="scope">
          <a>{{ scope.row.faultNum }}</a>
        </template>
      </el-table-column>
      <el-table-column label="等待处理量">
        <template slot-scope="scope">
          <a>{{ scope.row.waitDealNum }}</a>
        </template>
      </el-table-column>
      <el-table-column label="处理中量">
        <template slot-scope="scope">
          <a>{{ scope.row.dealingNum }}</a>
        </template>
      </el-table-column>
      <el-table-column label="处理成功量">
        <template slot-scope="scope">
      ...
      <el-table-column label="操作" align="center" width="280px" class-name="small-padding fixed-width" v-if="roles.includes('admin')">
        <template slot-scope="{row}">
          <el-button type="primary" size="mini" @click="handleDeal(row)">
            失败处理
          </el-button>
        </template>
      </el-table-column>
...
<script>
import { getErrInfoStat, updateStateToWaitDeal } from "@/api/logErrDeal";
...
export default {
  ...
  methods: {
    ...
    getList() {
      ...
      getErrInfoStat(this.listQuery).then(response => {
    ...
    handleDeal(row) {
      ...
      updateStateToWaitDeal(this.trendParam).then(() => {
        this.$message({
          type: "success",
          message: "修改状态为待处理成功"
        });
