<template>
  <div class="app-container">
    ...
    <vp-table :key="tableKey" v-loading="listLoading" :data="list" border fit highlight-current-row style="width: 100%;">
      ...
      <vp-table-column label="处理失败量">
        <template slot-scope="scope">
          <a>{{ scope.row.faultNum }}</a>
        </template>
      </vp-table-column>
      <vp-table-column label="等待处理量">
        <template slot-scope="scope">
          <a>{{ scope.row.waitDealNum }}</a>
        </template>
      </vp-table-column>
      <vp-table-column label="处理中量">
        <template slot-scope="scope">
          <a>{{ scope.row.dealingNum }}</a>
        </template>
      </vp-table-column>
      <vp-table-column label="处理成功量">
        <template slot-scope="scope">
      ...
      <vp-table-column label="操作" align="center" width="280px" class-name="small-padding fixed-width" v-if="roles.includes('admin')">
        <template slot-scope="{row}">
          <vp-button type="primary" size="mini" @click="handleDeal(row)">
            失败处理
          </vp-button>
        </template>
      </vp-table-column>
