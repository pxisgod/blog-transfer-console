<template>
  <div class="app-container" style="margin:24px 65px;">
    <page-head @back="goBack" content="日志统计详情"></page-head>
...
<script>
import { getLogStatTrend, getLogStat } from "@/api/logStat";
...
const statType = ["过滤量", "错误量", "成功量", "日志总量"];
...
export default {
  ...
  methods: {
    ...
    getList() {
      ...
      getLogStatTrend(this.logStatInfo).then(response => {
        if (response.data) {
          this.list = response.data;
          this.filterStatList = [];
          this.errStatList = [];
          this.successStatList = [];
          this.logNumStatList = [];
          this.xData = [];
          for (var index in this.list) {
            this.filterStatList.push(this.list[index].filterNum);
            this.errStatList.push(this.list[index].errNum);
            this.successStatList.push(this.list[index].successNum);
            this.logNumStatList.push(this.list[index].logNum);
